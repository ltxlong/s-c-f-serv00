...
# vless
V_WEBSITE=www.visa.com.sg
# vmess
VM_WEBSITE=www.visa.com.sg
...
mkdir -pv ${HOME}/s-c-f-serv00/ ; cd ${HOME}/s-c-f-serv00/
...
makeAndrun() {
    ...
    cat <<UiLgNoD-lIaMtOh | tee ${HOME}/s-c-f-serv00/config.json >/dev/null
{
  "log": {
    "disabled": false,
    "level": "info",
...
UiLgNoD-lIaMtOh
    ...
    cd ${HOME}/domains/${SERV00_DOMAIN}/sbox-c-f/ && npm install && \
    
    cd ${HOME}/s-c-f-serv00/
    ...
    # 节点链接
    VLESS_LINK_1="vless://$V_UUID_1@$V_WEBSITE:$CLOUDFLARED_PORT_V?encryption=none&security=tls&sni=$CLOUDFLARED_DOMAIN_V_1&fp=$BROWSER&type=$VV_TYPE&host=$CLOUDFLARED_DOMAIN_V_1&path=%2F$V_PATH_1%3Fed%3D2560#$SB_V_PROTOCOL_OUT_TAG_1"
    ...
    cat <<UiLgNoD-lIaMtOh | tee ${HOME}/s-c-f-serv00/client-nekobox-config.yaml >/dev/null
port: 7891
socks-port: 7892
mixed-port: 7893
...
UiLgNoD-lIaMtOh
    ...
    cat <<UiLgNoD-lIaMtOh | tee ${HOME}/s-c-f-serv00/client-sing-box-config.json >/dev/null
{
  "log": {
    "level": "debug",
    "timestamp": true
...
UiLgNoD-lIaMtOh
    ...
    cat <<UiLgNoD-lIaMtOh | tee ${HOME}/s-c-f-serv00/crontab >/dev/null
@reboot if ! pgrep -laf index.js >/dev/null; then node ${HOME}/domains/${SERV00_DOMAIN}/sbox-c-f/index.js >/dev/null 2>&1 & fi
$(crontab -l | sed '/index.js/d')
UiLgNoD-lIaMtOh
    crontab ${HOME}/s-c-f-serv00/crontab
    rm -fv ${HOME}/s-c-f-serv00/crontab

    # 检查写入之后的 crontab
    echo '写入之后的 crontab'
    crontab -l
    # 写入 list
    cat <<UiLgNoD-lIaMtOh | tee ${HOME}/domains/${SERV00_DOMAIN}/sbox-c-f/list >/dev/null
$VLESS_LINK_1 
$VMESS_LINK_2
UiLgNoD-lIaMtOh

    cp ${HOME}/domains/${SERV00_DOMAIN}/sbox-c-f/list ${HOME}/s-c-f-serv00/list

    # 写入 result.txt 字符画
    cat <<'UiLgNoD-lIaMtOh' | tee ${HOME}/s-c-f-serv00/result.txt >/dev/null
# ---------------------------------
...
UiLgNoD-lIaMtOh
    # 写入 result.txt
    cat <<UiLgNoD-lIaMtOh | tee -a ${HOME}/s-c-f-serv00/result.txt >/dev/null
    ！！！！！！！！！！！！注意！！！！！！！！！！！！！！！
    # 有时候？忽然连不上了
    # 执行以下命令查看进程是否启动？
    # sing-box-freebsd 进程
    ...
    if ! pgrep -laf index.js >/dev/null; then node ${HOME}/domains/${SERV00_DOMAIN}/sbox-c-f/index.js >/dev/null 2>&1 & fi
...
# 本地sing-box客户端配置文件位置 ${HOME}/s-c-f-serv00/client-sing-box-config.json
# 用于 sing-box 客户端配置文件拉取到本地 . 的命令如下
scp -P 22 $USERNAME@$HOSTNAME_DOMAIN:${HOME}/s-c-f-serv00/client-sing-box-config.json .

# 本地 mihomo 或 nekobox 等客户端配置文件配置文件位置 ${HOME}/s-c-f-serv00/client-nekobox-config.yaml
# 用于 mihomo 或 nekobox 等客户端配置文件拉取到本地 . 的命令如下
scp -P 22 $USERNAME@$HOSTNAME_DOMAIN:${HOME}/s-c-f-serv00/client-nekobox-config.yaml .
...
downloadFile() {
    ...
    tar zxvf $FILENAME
    mv -fv .$FILEPATH ${HOME}/s-c-f-serv00/sing-box-freebsd
    chmod -v u+x ${HOME}/s-c-f-serv00/sing-box-freebsd
    rm -rfv $FILENAME usr
    ...
    tar zxvf $FILENAME
    mv -fv ./$FILEPATH ${HOME}/s-c-f-serv00/cloudflared-freebsd
    chmod -v u+x ${HOME}/s-c-f-serv00/cloudflared-freebsd
